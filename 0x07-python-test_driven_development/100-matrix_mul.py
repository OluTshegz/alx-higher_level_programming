#!/usr/bin/python3
"""Task 6. Matrix multiplication:
    This module has/contains one/a function
    that multiplies 2 matrices
    Matrix multiplication - only Matrix product (two matrices)"""


def matrix_mul(m_a, m_b):
    """multiplies 2 matrices
    Args:
        m_a (list of lists of int/float): matrix to be multiplied
        m_b (list of lists of int/float): matrix to be multiplied
    Raises:
        TypeError: If m_a or m_b is not a list
        TypeError: If m_a or m_b is not a list of lists
        ValueError: If m_a or m_b is empty
        TypeError: If one element of list of lists is not int/float
        TypeError: If row of m_a or m_b are not the same size
        ValueError: If m_a and m_b cannot be multiplied
    Returns:
        A new list which is the multiplication of the two matrices"""

    if (type(m_a) is not list):
        raise TypeError("m_a must be a list")
    if (type(m_b) is not list):
        raise TypeError("m_b must be a list")

    if (any(type(row) is not list for row in m_a)):
        raise TypeError("m_a must be a list of lists")
    if (any(type(row) is not list for row in m_b)):
        raise TypeError("m_b must be a list of lists")

    if (m_a == [] or m_a == [[]]):
        raise ValueError("m_a can't be empty")
    if (m_b == [] or m_b == [[]]):
        raise ValueError("m_b can't be empty")

    if (any(type(elem) not in [int, float]
            for row in m_a for elem in row)):
        raise TypeError("m_a should contain only integers or floats")
    if (any(type(elem) not in [int, float]
            for row in m_b for elem in row)):
        raise TypeError("m_b should contain only integers or floats")

    if not all(len(row) == len(m_a[0]) for row in m_a):
        raise TypeError("each row of m_a must be of the same size")
    if not all(len(row) == len(m_b[0]) for row in m_b):
        raise TypeError("each row of m_b must be of the same size")

    if (len(m_a[0]) != len(m_b)):
        raise ValueError("m_a and m_b can't be multiplied")

    m_1 = []
    for i in range(len(m_b[0])):
        my_row = []
        for j in range(len(m_b)):
            my_row.append(m_b[j][i])
        m_1.append(my_row)

    m_2 = []
    for row in m_a:
        my_row = []
        for column in m_1:
            product = 0
            for m in range(len(m_1[0])):
                product += row[m] * column[m]
            my_row.append(product)
        m_2.append(my_row)

    return m_2
